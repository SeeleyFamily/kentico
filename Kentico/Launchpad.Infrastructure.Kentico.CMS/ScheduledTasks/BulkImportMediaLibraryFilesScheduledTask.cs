...
{
	public class BulkImportMediaLibraryFilesScheduledTask : ITask
	{
		#region Fields
		...
		public BulkImportMediaLibraryFilesScheduledTask()
		{
		...
		public string Execute(TaskInfo ti)
		{
			...
			Service.Resolve<IEventLogService>().LogInformation("BulkImportMediaLibraryFilesScheduledTask", "Execute", details);
			...
			if (new List<string>() { "LOCAL", "DEV" }.Contains(appEnvironment))
			{
				try
				{
					mediaLibraryFilesModuleService.BulkImportMediaLibraryFiles();
				}
				catch (Exception e)
				{
					result = e.Message;
